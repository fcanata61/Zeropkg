...
from registry import register_package, list_installed, unregister_package
...
def cmd_remove(args):
    name = args.name
    ok = unregister_package(name)
    if ok:
        logger.info(f"Pacote {name} removido do registro.")
        # Aqui no futuro: percorrer installed_files e remover do sistema
    else:
        logger.error(f"Pacote {name} não encontrado no registro.")
...
def main():
    ...
    # remove (abreviação: rm)
    rm = sub.add_parser("remove", aliases=["rm"], help="remover pacote do registro")
